...
`BookContent`는 `sectionId`를 받아 `bookStructure.ts`의 `getSectionById()`로 챕터를 확인한 뒤, 챕터 ID → 챕터 컴포넌트로 직접 분기(switch-like)한다. 모든 챕터 페이지는 아래 동일한 prop 인터페이스를 사용한다:

```ts
interface Props {
  sectionId: string   // 현재 활성 섹션 ID
  lang: Lang          // 'ko' | 'en'
  onNavigate: (id: string) => void  // TOC 이동
}
```
